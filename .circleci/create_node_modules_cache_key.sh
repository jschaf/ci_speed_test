...
hash_file='.circleci/node_modules_cache_key'
patches_hashes="$(find ./patches -type f -print0 | xargs -0 sha1sum | sort)"
package_lock_hash="$(sha1sum package-lock.json)"
echo "# Hashes for node_modules cache
NPM version: $(npm --version)
Node version: $(node --version)
${package_lock_hash}
${patches_hashes}" > ${hash_file}

printf 'Collected hashes from all files in ./patches and from package-lock.json:\n\n'
cat "${hash_file}"
printf '\nWriting current node_modules cache key to .circleci/node_modules_cache_key.\n'
